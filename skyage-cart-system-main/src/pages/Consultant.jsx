  ...
  return (
    <main className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
      ...
      <div className="flex flex-col md:flex-row items-stretch gap-0">
        <form onSubmit={handleSubmit} className="w-full md:w-1/2 bg-white p-3 sm:p-6 rounded shadow space-y-3 sm:space-y-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {status && typeof status === 'object' && status.ok && <span className="text-green-600 text-xs sm:text-sm">{t('sent')} (id: {status.id})</span>}
            {status && typeof status === 'object' && !status.ok && <span className="text-red-600 text-xs sm:text-sm">{t('errorPrefix')} {status.error}</span>}
          </div>
        </form>
